...
return try 
{if (fn:empty($user) or fn:empty($password) or not(xdmp:login($user,$password,true()))) then xdmp:redirect-response('/login')
	else 
	let $_:=xdmp:add-response-header('X-ML-User',xdmp:get-current-user())
